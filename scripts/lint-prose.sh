...
# Pinned, and the same version the workflow's cache key implies. Vale ships a
# single static binary and no JDK, unlike ltex-ls-plus below — this scaffold
# has no Go toolchain to `go install` it with, so this is a plain download
# instead, the same shape as the ltex-ls-plus fetch in lint-mechanics.sh.
VALE_VERSION=3.19.0

CACHE="${XDG_CACHE_HOME:-$HOME/.cache}/vale/$VALE_VERSION"
if [ ! -x "$CACHE/vale" ]; then
  echo "Fetching vale $VALE_VERSION"
  mkdir -p "$CACHE"
  url="https://github.com/errata-ai/vale/releases/download/v${VALE_VERSION}/vale_${VALE_VERSION}_Linux_64-bit.tar.gz"
  curl -fsSL "$url" | tar -xz -C "$CACHE" vale
fi
